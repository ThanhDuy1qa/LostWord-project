...
import { useFriends } from '../hooks/useFriends'; // Import logic từ Hook

const FriendList = () => {
  // Lấy toàn bộ logic đã đóng gói
  const { filteredFriends, searchTerm, setSearchTerm, loading } = useFriends();

  if (loading) {
    return <div className="min-h-screen bg-[#0f0f12] text-white flex justify-center items-center text-xl">Đang tải dữ liệu...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f0f12] text-white p-5">
      ...
      {/* Grid danh sách */}
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 gap-3 sm:gap-4 px-2 sm:px-4">
